...
def generate_uniform_points_on_cube(domain_intervals, N_each_face):
    ...
    if d == 1:
        ...
        for i in range(d):
            points[2 * i * N_each_face:(2 * i + 1) * N_each_face, :] = numpy.insert(
                generate_uniform_points_in_cube(numpy.delete(domain_intervals, i, axis=0), N_each_face), i,
                values=domain_intervals[i, 0] * numpy.ones((1, N_each_face)), axis=1)
            points[(2 * i + 1) * N_each_face:(2 * i + 2) * N_each_face, :] = numpy.insert(
                generate_uniform_points_in_cube(numpy.delete(domain_intervals, i, axis=0), N_each_face), i,
                values=domain_intervals[i, 1] * numpy.ones((1, N_each_face)), axis=1)
        return points
...
def generate_uniform_points_on_cube_time_dependent(domain_intervals, time_interval, N_each_face, N_initial_time_slice,
                                                   time_condition_type='initial'):
    d = domain_intervals.shape[0]
    ...
    for i in range(1, 1 + d):
        points_bd[2 * (i - 1) * N_each_face:(2 * i - 1) * N_each_face, :] = numpy.insert(
            generate_uniform_points_in_cube(numpy.delete(whole_intervals, i, axis=0), N_each_face), i,
            values=whole_intervals[i, 0] * numpy.ones((1, N_each_face)), axis=1)
        points_bd[(2 * i - 1) * N_each_face:2 * i * N_each_face, :] = numpy.insert(
            generate_uniform_points_in_cube(numpy.delete(whole_intervals, i, axis=0), N_each_face), i,
            values=whole_intervals[i, 1] * numpy.ones((1, N_each_face)), axis=1)
    if time_condition_type == 'initial':
        points_int = numpy.insert(
            generate_uniform_points_in_cube(numpy.delete(whole_intervals, 0, axis=0), N_initial_time_slice), 0,
            values=whole_intervals[0, 0] * numpy.ones((1, N_initial_time_slice)), axis=1)
    elif time_condition_type == 'terminal':
        points_int = numpy.insert(
            generate_uniform_points_in_cube(numpy.delete(whole_intervals, 0, axis=0), N_initial_time_slice), 0,
            values=whole_intervals[0, 1] * numpy.ones((1, N_initial_time_slice)), axis=1)
...
def generate_uniform_points_on_sphere_time_dependent(d, R, time_interval, N_boundary, N_initial_time_slice):
    if d == 1:
        return generate_uniform_points_on_cube_time_dependent(numpy.array([[-R, R]]), time_interval,
                                                              int(round(N_boundary / 2)), N_initial_time_slice,
                                                              time_condition_type='initial')
...
def generate_x_plot(domain_intervals, N_plot):
    dim = domain_intervals.shape[0]
    grid = numpy.zeros((N_plot, dim))
    for i in range(dim):
        grid[:, i] = numpy.linspace(domain_intervals[i, 0], domain_intervals[i, 1], N_plot)

    x_plot = numpy.zeros((N_plot**dim, dim))

    for index in range(N_plot**dim):
        pos = numpy.zeros((dim + 1, )).astype(int)
        reminder = index
        for i in range(dim):
            pos[dim-1-i] = reminder % N_plot
            reminder = numpy.floor_divide(reminder - pos[dim-i], N_plot)

        for i in range(dim):
            x_plot[index, i] = grid[pos[i], i]
    return x_plot

# generate a list of learning rates
def generate_learning_rates_stage(highest_lr_pow,lowest_lr_pow,total_iterations,ratio_get_to_the_lowest,n_stage):
