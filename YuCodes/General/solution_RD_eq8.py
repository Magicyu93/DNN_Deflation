...
def res_bd(model, tensor_x_batch):
    ...
    res_bd_result = torch.zeros(tensor_x_batch.shape)
    ...
    for i in range(dim):
        ...
        grad = 1/scale * (model(tensor_x_batch[count_start:count_end, :] + h*ei)
                          - model(tensor_x_batch[count_start:count_end, :] - h*ei))/2/h
        res_bd_result[count_start:count_end, i] = grad
    return res_bd_result


# right now only works for the 1d case
def res_bd_1d(model, tensor_x2_train):
